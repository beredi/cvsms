...
@section('content')
    ...
    <div class="row">
        <div class="col-lg-5 col-md-12">
            <h2>{{__('messages.admin.menu.stock.update-record')}}</h2>
            @if ($errors->any())
            ...
            <form method="POST" action="{{ route('stock-item.update', ['stockItem' => $item->id]) }}">
                @csrf
                @method('PATCH')
                ...
                <div class="form-group">
                    ...
                    <div class="control-group">
                        <input type="number" name="purchase_price" class="form-control @error('purchase_price') is-invalid @enderror" value="{{ $item->purchase_price }}" required>
                    </div>
                </div>
                <div class="form-group">
                    <label for="price" class="required">{{ __('messages.admin.menu.stock.item.price') }}</label>
                    <div class="control-group">
                        <input type="number" name="price" class="form-control @error('price') is-invalid @enderror" value="{{ $item->price }}" required>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="number-input">
                        ...
                        <input type="number" min="0" name="on_stock" class="form-control @error('on_stock') is-invalid @enderror" value="{{ $item->on_stock }}" required>
                ...
                <div class="form-group">
                        <button type="submit" class="btn btn-primary">
                            {{ __('messages.admin.menu.stock.update-record') }}
                        </button>
                </div>
            </form>
        </div>
